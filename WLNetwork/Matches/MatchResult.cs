			...
			if (MatchCounted) {
				...
				Mongo.Users.Update (
					Query.In ("steam.steamid",
						Players.Where (m => m.Team == MatchTeam.Radiant && !m.IsLeaver).Select (m => new BsonString (m.SID)).ToArray ()),
					radUpdate, UpdateFlags.Multi);
				Mongo.Users.Update (
					Query.In ("steam.steamid",
						Players.Where (m => m.Team == MatchTeam.Dire && !m.IsLeaver).Select (m => new BsonString (m.SID)).ToArray ()),
