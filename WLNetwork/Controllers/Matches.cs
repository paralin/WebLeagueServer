...
namespace WLNetwork.Controllers
{
    ...
    public class Matches : WebLeagueController
    {
        ...
        public string CreateMatch(MatchCreateOptions options)
        {
            ...
            if (start > DateTime.UtcNow) return $"The league '{league.Name}' starts in {(DateTime.UtcNow - start).Humanize(2)}.";
        ...
        public string StartChallenge(Challenge target)
        {
            ...
            var start = league.Seasons[(int) league.CurrentSeason].Start;
            if (start > DateTime.UtcNow && target.MatchType != MatchType.OneVsOne) return $"The league '{league.Name}' starts in {(start-DateTime.UtcNow).Humanize(2)}.";
